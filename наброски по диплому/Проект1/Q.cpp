...
{
	double eps1 = 10E-4, eps2 = 10E-3, alpha = 1, a_bot=0, a_top=0; 
	double thet1 = 2, thet2 = 0.5, scal =  d_f(xk, dim).scal_mul(dk);
	vect xk_adk(dim);
	...
	while (stop_index < 1E4)
	{
		stop_index++;
		xk_adk = (dk*alpha) + xk;
		if (f_x(xk_adk) - f_x(xk) - eps1 * alpha * scal <= 0) // value1 - value2 < 0.0...01 ?
		{
...
vect QNM::QNM_solution()
{
	...
	max_iter = 0;
		...
		//Wulf
			...
			//alpha = find_min(x_cur,d_k,-10,10);
			...
			for(int i = 0; i < dim; i++)
				...
				//fout <<"grad = "<< d_f(x_new,dim).check_norm() << " \n";
				printf("\n");
		//Qk+1
